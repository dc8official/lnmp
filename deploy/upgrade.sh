#!/usr/bin/env bash
# ==============================================================================
# LNMP Network Monitoring Platform v3.1.3s - Automated Upgrade Utility
# ==============================================================================
...
echo -e "${BLUE}========================================================================${NC}"
echo -e "${BLUE}    LNMP Network Monitoring Platform v3.1.3s - Upgrade Utility           ${NC}"
echo -e "${BLUE}========================================================================${NC}"
...
if [[ ${DRY_RUN} -eq 0 ]]; then
    ...
    if [[ -d "${PROJECT_ROOT}/.git" ]]; then
        ...
        git checkout "${UPGRADE_BRANCH}" 2>/dev/null || git checkout v3.1.3s 2>/dev/null || true
        git pull origin "${UPGRADE_BRANCH}" 2>/dev/null || git pull origin v3.1.3s 2>/dev/null || git pull || echo -e "${YELLOW}[WARN] Git pull finished with non-zero exit code. Proceeding with existing files.${NC}"
        SOURCE_DIR="${PROJECT_ROOT}"
    # Case B: Running from /opt/netmon/noop or non-git directory -> clone directly from remote
        ...
        if git clone --depth 1 --branch "${UPGRADE_BRANCH}" "${REPO_URL}" "${STAGE_DIR}" 2>/dev/null || \
           git clone --depth 1 --branch "v3.1.3s" "${REPO_URL}" "${STAGE_DIR}" 2>/dev/null || \
...
echo -e "\n${GREEN}========================================================================${NC}"
echo -e "${GREEN}   [UPGRADE COMPLETE] LNMP v3.1.3s Platform upgraded successfully!       ${NC}"
