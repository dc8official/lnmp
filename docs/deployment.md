# LNMP Deployment & Operations Guide — Version 3.1.7s

This guide details the procedures for installing, maintaining, and upgrading the Network Monitoring Platform (LNMP) v3.1.7s on a production Linux server.
...
4. Generates `/etc/netmon/netmon.env` and `/etc/netmon/config.toml` with v3.1.7s defaults.
...
## 3. Upgrading to Version 3.1.7s (Zero Historical Data Loss)

To perform an in-place upgrade to v3.1.7s:
...
LNMP v3.1.7s dispatches outbound incident notifications directly to external endpoints. Enterprise network administrators must configure egress firewall policies allowing outbound traffic from the LNMP host:
